 ...
 *
 * TODO: Outsource functionality and validation into eventsModel->setUpdateEvent()
 * TODO: Integrate with akteure- and festivals-models
 */

namespace Drupal\AaeData;

Class eventformular extends events {
  ...
  private function eventCheckPost() {
    ...
    /*if ($this->start > $this->ende) {
     $this->fehler['ende'] = t("Das End- darf nicht vor dem Start-Datum liegen!");
    }*/
...
/*print_r($this->bild); echo ' b '.$this->bild; exit();
  ...
  if ($this->eventRecurres == 'on' && $status->recurring_event_type < 6 && (($status->recurring_event_type != $this->eventRecurringType) 
   ...
   $this->removeEventChildren($this->event_id);
   $recurresTill = (!empty($this->eventRecurresTill)) ? (new \DateTime($this->eventRecurresTill))->format('Y-m-d') : NULL;
   $this->addEventChildren($this->event_id, $this->eventRecurringType, $startQuery, $endeQuery, $recurresTill);
  ...
  private function eventGetFields() {
   ...
   $resultEvent = reset($this->GetEvents(array('EID' => $this->event_id), 'complete'));
  ...
  private function eventDisplay() {
    ...
    $this->ownedFestivals = $this->userHasFestivals($this->user_id);
